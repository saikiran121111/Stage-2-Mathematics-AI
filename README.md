...
In AI, if we want to represent our employee Alice using only her numerical data (ID: 1, Salary: 50000), her "vector" is just [1, 50000].

NumPy Code: alice_vector = np.array([1, 50000])

2. What is a Matrix?
A matrix is a 2-dimensional grid (or table) of numbers. If a vector is a single row, a matrix is the whole spreadsheet. If we put Alice and Bob's numerical data together, we get a matrix.
...
# How the math works: (2 * 4) + (3 * 5) = 8 + 15 = 23
result = np.dot(a, b)
